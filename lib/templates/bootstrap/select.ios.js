...
class CollapsiblePickerIOS extends React.Component {
  ...
  onChange(...args){
      this.toggle();
      this.props.locals.onChange(...args);
  }
  ...
  render() {
    ...
    options.sort((a, b)=>{
        ...
        var aText=a.text.toLowerCase().replace(/\s/g, ''),
            bText=b.text.toLowerCase().replace(/\s/g, '');
    ...
    options = options.map(({value, text}) => <Picker.Item key={value} value={value} label={text} />);
